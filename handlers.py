    ...
    with next(get_db()) as db:
        try:
            ...
            if existing_active_link:
                ...
                bot_instance_for_persona = db.query(BotInstance).filter(
                    BotInstance.persona_config_id == persona.id
                ).first()
                if bot_instance_for_persona and old_bot_instance_id == bot_instance_for_persona.id:
    ...
    try:
        with next(get_db()) as db:
            ...
            try:
                 ...
                 logger.warning(f"Invalid JSON for persona {persona_id}, resetting moods.")
                 current_moods = {}
...
async def delete_mood_confirmed(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not mood_name or not persona_id or not data.endswith(mood_name):
