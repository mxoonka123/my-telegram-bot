...
async def handle_media(update: Update, context: ContextTypes.DEFAULT_TYPE, media_type: str) -> None:
    ...
    with get_db() as db:
        try:
            ...
            # Проверка месячных лимитов сообщений
            # Для бесплатных пользователей проверяем, не превышен ли лимит
            if not owner_user.is_active_subscriber:
                if owner_user.monthly_message_count >= FREE_USER_MONTHLY_MESSAGE_LIMIT:
                    logger.info(f"Owner {owner_user.telegram_id} exceeded monthly message limit ({owner_user.monthly_message_count}/{FREE_USER_MONTHLY_MESSAGE_LIMIT}) for media.")
                    await send_limit_exceeded_message(update, context, owner_user)
                    return
                
                # Увеличиваем счетчик сообщений
                owner_user.monthly_message_count += 1
                limit_state_updated = True
                logger.debug(f"Incremented monthly message count for user {owner_user.telegram_id} to {owner_user.monthly_message_count}")
            else:
                limit_state_updated = False
                logger.debug(f"Skipping message limit check for premium user {owner_user.telegram_id}")
...
async def reset(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    logger.info(f"CMD /reset is an alias for /clear. Forwarding request.")
