...
from config import (
    ...
    ADMIN_USER_ID, CHANNEL_ID # <<< ДОБАВЛЕНО: Импорт CHANNEL_ID
)
from db import (
...
async def check_channel_subscription(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    try:
        ...
        # Bot might not be an admin in the channel, or channel is inaccessible
        ...
        # Consider allowing access if the check fails due to bot permissions,
        # or denying access as a security measure. Let's deny for now.
        ...
        # Deny access on other Telegram errors
        await update.effective_message.reply_text(escape_markdown_v2("Произошла ошибка при проверке подписки. Попробуйте позже."))
        return False
    except Exception as e:
        logger.error(f"Unexpected error checking subscription for user {user_id} in channel {CHANNEL_ID}: {e}", exc_info=True)
        # Deny access on unexpected errors
        return False
...
async def send_subscription_required_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if channel_username:
        ...
        keyboard = [[InlineKeyboardButton(button_text, url=f"https://t.me/{channel_username}")]]
         ...
         # Cannot generate a direct link from ID easily without invite link, inform user
         subscribe_text_raw += " Пожалуйста, найдите канал в поиске или через описание бота."
         # No button if we can't link reliably
    else:
...
async def send_limit_exceeded_message(update: Update, context: ContextTypes.DEFAULT_TYPE, user: User):
    ...
    # <<< ИЗМЕНЕНО: Аккуратное экранирование с сохранением ** >>>
    text_to_send = (
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    reply_markup = None # <<< ИЗМЕНЕНО: Инициализация разметки
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
     ...
     await context.bot.send_chat_action(chat_id=chat_id, action=ChatAction.TYPING)
     # <<< Используем raw string r"""...""" для упрощения экранирования >>>
...
/unmutebot \- разрешить личности отвечать в чате
     """
     try:
         # Отправляем как есть, т.к. уже содержит Markdown V2
...
async def mood(update: Update, context: ContextTypes.DEFAULT_TYPE, db: Optional[Session] = None, persona: Optional[Persona] = None) -> None:
    ...
    # +++ ПРОВЕРКА ПОДПИСКИ +++
    # Не делаем проверку, если это callback от кнопки, т.к. пользователь уже взаимодействует
    if not is_callback:
        if not await check_channel_subscription(update, context):
            await send_subscription_required_message(update, context)
            return
    ...
    try:
        ...
        if local_persona is None:
            ...
            if not persona_info_tuple:
                error_reply_target = update.callback_query if is_callback else message_or_callback_msg
                try:
                    if is_callback: await update.callback_query.answer("Нет активной личности", show_alert=True)
                    await error_reply_target.reply_text(error_no_persona, reply_markup=ReplyKeyboardRemove())
        ...
        if chat_bot_instance.is_muted:
            ...
            try:
                 # Answer callback first if applicable
        ...
        if is_callback and update.callback_query.data.startswith("set_mood_"):
             ...
             # Example: set_mood_радость_123 -> mood_arg = "радость"
             if len(parts) >= 3:
                  # Reconstruct mood name if it contained underscores
                  mood_arg_lower = "_".join(parts[2:-1]).lower() # Everything between set_mood_ and _personaId
        ...
        if target_mood_original_case:
             ...
             # Show keyboard
             # Ensure callback data is unique and includes persona ID for robustness
         ...
         reply_target = update.callback_query if is_callback else message_or_callback_msg
         try:
...
async def reset(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    success_reset_fmt = escape_markdown_v2("память личности '{persona_name}' в этом чате очищена.") # Placeholder
    ...
    with next(get_db()) as db:
        try:
            ...
            final_success_msg = success_reset_fmt.format(persona_name=persona_name_escaped)
...
async def create_persona(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    with next(get_db()) as db:
        try:
            ...
            # Используем selectinload для persona_configs для эффективности can_create_persona
...
async def my_personas(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    info_no_personas_fmt = r"у тебя пока нет личностей \(лимит: {count}/{limit}\)\.\nсоздай: /createpersona <имя>" # Placeholder
    ...
    try:
        with next(get_db()) as db:
            ...
            if not personas:
                # Экранируем числа перед форматированием
                final_no_personas_msg = info_no_personas_fmt.format(
                    count=persona_count,
                    limit=persona_limit
                )
            ...
            # Экранируем числа для заголовка
            text = info_list_header_fmt.format(count=persona_count, limit=persona_limit)
...
async def add_bot_to_chat(update: Update, context: ContextTypes.DEFAULT_TYPE, persona_id: Optional[int] = None) -> None:
    ...
    success_added_fmt = escape_markdown_v2("✅ личность '{name}' (id: ") + "`{id}`" + escape_markdown_v2(") активирована в этом чате! Память очищена.") # Placeholder
    ...
    with next(get_db()) as db:
        try:
            ...
            # user = get_or_create_user(db, user_id, username) # user уже загружен через persona.owner
            ...
            if not bot_instance:
                 # create_bot_instance коммитит внутри, что может быть проблемой, если дальше ошибка
                 # Лучше добавить, но закоммитить в конце
            ...
            if chat_link:
                 # Контекст уже очищен внутри link_bot_instance_to_chat
                 final_success_msg = success_added_fmt.format(name=persona_name_escaped, id=local_persona_id)
...
async def handle_callback_query(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Проверяем подписку только для действий, инициирующих важную логику,
    # а не для простых навигационных кнопок или отмены
    needs_subscription_check = True
    # Белый список callback'ов, НЕ требующих проверки подписки:
    no_check_callbacks = (
        "cancel_edit", "edit_persona_back", "edit_moods_back_cancel",
        "delete_persona_cancel", "view_tos", "subscribe_info",
        "show_help", # <<< ДОБАВЛЕНО: Кнопка помощи не требует подписки
        "dummy_" # Заглушки тоже не требуют
    )
    ...
    if data.startswith("set_mood_"):
        # mood() обработает ответ на callback
        ...
        # add_bot_to_chat() обработает ответ на callback
        await add_bot_to_chat(update, context)
    # <<< ИЗМЕНЕНО: Обработка кнопки /help >>>
    elif data == "show_help":
        await query.answer()
        await help_command(update.callback_query, context) # Вызываем help_command
...
async def _start_edit_convo(update: Update, context: ContextTypes.DEFAULT_TYPE, persona_id: int) -> int:
    ...
    prompt_edit_fmt = r"редактируем **{name}** \(id: `{id}`\)\nвыбери, что изменить:" # Placeholder
...
async def edit_persona_choice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Используем raw string
    prompt_edit_value_fmt = r"отправь новое значение для **{field_name}**.\n_текущее:_\n`{current_value}`" # Placeholder
    prompt_edit_max_msg_fmt = r"отправь новое значение для **{field_name}** \(число от 1 до 10\):\n_текущее: {current_value}_" # Placeholder
...
async def edit_field_update(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Используем raw string
    success_update_fmt = r"✅ поле **{field_name}** для личности **{persona_name}** обновлено!" # Placeholder
    prompt_next_edit_fmt = r"что еще изменить для **{name}** \(id: `{id}`\)?" # Placeholder
...
async def edit_max_messages_update(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    error_general = escape_markdown_v2("❌ непредвиденная ошибка при обновлении.")
    # Используем raw string
    success_update_fmt = r"✅ макс\. сообщений в ответе для **{name}** установлено: **{value}**" # Placeholder
    prompt_next_edit_fmt = r"что еще изменить для **{name}** \(id: `{id}`\)?" # Placeholder
...
async def _get_edit_moods_keyboard_internal(persona_config: PersonaConfig) -> List[List[InlineKeyboardButton]]:
     ...
     if moods:
         ...
         for mood_name in sorted_moods:
              # Создаем "безопасное" имя для callback_data (латиница, цифры, _)
              safe_mood_name = re.sub(r'\W+', '_', mood_name).lower()
              # Обрезаем, если слишком длинное для callback_data (Telegram limit ~64 bytes)
              safe_mood_name = safe_mood_name[:30]
              if not safe_mood_name: continue # Пропускаем, если имя стало пустым

              # Используем оригинальное имя для отображения
              display_name = mood_name.capitalize()

              # Сохраняем ОРИГИНАЛЬНОЕ имя в callback data, чтобы точно знать, что редактировать/удалять
              # Используем URL-кодирование для безопасности, если имя содержит спецсимволы
...
async def edit_moods_menu(update: Update, context: ContextTypes.DEFAULT_TYPE, persona_config: Optional[PersonaConfig] = None) -> int:
    ...
    # Используем raw string
    prompt_mood_menu_fmt = r"управление настроениями для **{name}**:"
...
async def edit_mood_choice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    prompt_new_prompt_fmt = r"редактирование настроения: **{name}**\n\n_текущий промпт:_\n`{prompt}`\n\nотправь **новый текст промпта**:" # Placeholder
    prompt_confirm_delete_fmt = r"точно удалить настроение **'{name}'**\?" # Placeholder
