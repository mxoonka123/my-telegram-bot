...
def handle_telegram_webhook(token: str):
    ...
    try:
        ...
        # 0) Главный бот: игнорируем ВСЕ апдейты (полностью молчим)
        if main_bot_id and str(main_bot_id) == str(current_bot_id or ''):
            flask_logger.info(
                f"skip ALL updates for main bot @{bot_instance.telegram_username} (id={current_bot_id})"
            )
            return Response(status=200)
