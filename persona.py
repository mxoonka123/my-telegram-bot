...
class Persona:
    ...
    def _format_media_prompt(self, media_type_text: str) -> Optional[str]:
        ...
        if media_type_text == "фото":
            ...
            prompt_parts.append(
                "Пользователь прислал голосовое сообщение. Тебе предоставлена расшифровка этого сообщения. "
                "После `[голосом]:` будет следовать текст, который был сказан в голосовом сообщении. "
                "Если текст вместо расшифровки содержит '[...]голосовое сообщение...', значит расшифровать сообщение не удалось. "
                "Ответь на содержание голосового сообщения от своего лица, учитывая свой стиль и настроение (если оно указано)."
            )
